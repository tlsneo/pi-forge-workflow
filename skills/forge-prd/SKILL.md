...
The PRD covers problem, solution, goals, non-goals, actors, focused user stories, stable Acceptance IDs, happy/error/edge behavior, design decisions, `path#Symbol` impact evidence, test seams, risks, migration, rollback, open questions, and a Delivery Plan. Close the supported input domain for every new or changed public Interface: specify omitted, null, non-string, malformed, and compatibility behavior when supported, or explicitly place unsupported inputs in non-goals with no guaranteed behavior. Do not leave the same input both unsupported and open. Each Delivery Boundary freezes its delivery goal and scope as well as outcome and traceability, so the Issue phase cannot widen them. Use one `DB-01` boundary by default. Split into `DB-02...` only when each outcome can be independently implemented, verified, delivered, rolled back, and closed. Delivery-boundary choices belong in the PRD Decision Frontier and use the same Grilling protocol when user judgment is required. For each boundary, build `verification` deterministically in this exact order: every verification line from its owned Acceptance IDs, followed by the verification line of every named Test Seam, with duplicates removed while preserving first occurrence. Forge rejects an empty, incomplete, extra, duplicated, or reordered boundary verification list before Review so every accepted PRD can materialize Issues without another LLM proposal.
