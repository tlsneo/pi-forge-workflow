...
A Worker must call `task_resume`, follow only the exact versioned contract path frozen in its Binding, such as `TASK-V001.md`, and any frozen Correction Context returned by Runtime. It executes every `BP-xx` Step in order, obeys Expected Patch Shape, Forbidden Changes, Stop Conditions, and the fixed Minimal Implementation and Proportionality Policies, checkpoints semantic progress, maps every Step to concrete Handoff Evidence, calls `task_handoff` once, and stops. Fallback is default-deny. Every added guard, compatibility path, abstraction, dependency, check, and file must already be traceable in the frozen contract; optional confidence and future flexibility stay outside the patch. Cohesive behavior belongs in its owning Module rather than an app/composition-root file, but file count is not a goal and one-function pass-through Modules are forbidden. Worker-reported command results are advisory; the coordinator reruns every frozen verification command authoritatively.

Wait for the Nicobailon `subagent:async-complete` lifecycle event after a Worker or Auditor starts, then call `forge_run_continue` again. Recognizable `503`, safe RPC timeout, transport, or terminated lifecycle failures are persisted as `infrastructure_retry_scheduled` with a separate bounded retry budget; they do not consume semantic Worker or Reviewer attempts. A timeout after an RPC v1 spawn request was emitted has an unknowable outcome, so Forge records `infrastructure_spawn_outcome_unknown` and fails closed instead of risking a duplicate live Workflow. Exhaustion enters `infrastructure_failed`, not a product Blocker. Do not manually edit state, commit Worker changes, or invent a new dependency.
...
For the first Standard or High Assurance cycle, Runtime automatically starts three independent read-only `forge-reviewer` Bindings. Each receives one compact immutable Axis Surface, its Surface Hash, exact Task IDs, changed files, and only the evidence assigned to that Axis:

- Standards;
- Acceptance / Integration;
- Architecture / Minimality.

Each Auditor calls `forge_run_audit_submit` exactly once with its Axis Surface Hash. Runtime rejects a stale hash. It may report a Blocker only with evidence containing at least one exact repository path, `path#line` citation, or `path#symbol` Seam, a violated frozen Acceptance or documented hard rule, a reachable failure, action-changing verification, and the smallest sufficient resolution. Auditors must not duplicate another Axis or re-check settled evidence outside their Surface. Optional confidence and preferences remain Warnings or Notes; passing with no Findings is valid. Three passed axes—including hash-verified carried reviews—create `runtime/audits/issue-final.json` and move the Issue to `completed`.
