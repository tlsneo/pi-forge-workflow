...
Wait for the lifecycle event after a Worker or Auditor starts, then call `forge_run_continue` again. Recognizable `503`, safe RPC timeout, transport, or terminated lifecycle failures are persisted as `infrastructure_retry_scheduled` with a separate bounded retry budget; they do not consume semantic Worker or Reviewer attempts. A timeout after the spawn request was emitted has an unknowable outcome under pi-subagents RPC v2, so Forge records `infrastructure_spawn_outcome_unknown` and fails closed instead of risking a duplicate live Agent. Exhaustion enters `infrastructure_failed`, not a product Blocker. Do not manually edit state, commit Worker changes, or invent a new dependency.
